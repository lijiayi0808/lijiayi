 function [P_e_12,P_e_13,P_e_cut1,Obj_MG1,P_loss1,total_loss1,CVaR1]=Fun_MG1(P12d,P13d,lambda_e_12,lambda_e_13,p)
%商场的分布式优化迭代模型
%% 决策变量初始化
L_e=sdpvar(1,24);       %需求响应后实际的电负荷
P_e_cut1=sdpvar(1,24);  %可削减电负荷
...
P_e_12=sdpvar(1,24);    %1向2交互的电功率
P_e_13=sdpvar(1,24);    %1向3交互的电功率

%% 导入电负荷和电网购电电价
%商场
L_e0=[902, 946, 912, 922, 916, 914, 942, 1290,1598,2220,2248,2304,...
      2232,2282,2296,2380,2426,2486,2430,2268,2166,1008,978, 904]; 

Predict_pv=[0,0,0,0,0,300,675,1139,1876,2173,2390,2311,...
            2332,2399,2331,2278,1735,1400,730,0,0,0,0,0];
...
Tset = ([22.2,22.3,22.4,22.2,22.1,22.1,22.0,20.4,20.2,20.2,20.4,20.8,...
...
%虚拟储能容量
for t=1:24
    Esum(t) = R_in*c_in*(Tmax-Tset(t))*(V1*8); %向下调度
    Es(t) = R_in*c_in*(Tset(t)-Tmin)*(V1*8);   %向上调度
end
%% 
beta=0.1;
%电负荷需求响应部分
for t=1:24
    C=[C,
       L_e(t)==L_e0(t)+P_e_cut1(t),                  %电负荷功率平衡约束
       -(-0.1*beta+1.01)*Es(t)/3600000<=P_e_cut1(t)<=(-0.1*beta+1.01)*Esum(t)/3600000, %可削减电功率上下限约束
      ];
end
...
for t=1:24
    C=[C,
       P_e_pv(t)+P_buy(t)+P_batd(t)==P_batc(t)+L_e(t)+P_e_12(t)+P_e_13(t)+P_sell(t),
...
alpha1=0.95;
% 固定惩罚电价（每个时段相同）
lambda_loss1_t = 1 * ones(T, 1);  % 惩罚电价向量

% 给定的实际响应功率偏差数据（24个时段）
base_power_deviation1 = (-0.1*beta+1.01)*[315, 323, 330, 315, 308, 308, 300,...
                        180, 165, 165, 180, 210, 188, 173,...
                        195, 180, 210, 218, 218, 248, 270,...
                        263, 315, 338];  % 正整数值

% 生成场景数据（在基准值上添加±10%的随机波动）
P_loss1 = zeros(Z, T, N_w);  % 功率偏差矩阵
rng(42);  % 设置随机种子确保结果可重现
for k = 1:N_w
    % 在基准值上添加随机波动（90%~110%）
    fluctuation = 0.9 + 0.2 * rand(1, T);  % 生成[0.9, 1.1]的随机波动
    P_loss1(1, :, k) = base_power_deviation1 .* fluctuation;
end
...
%% 计算风险指标
[VaR1, CVaR1, total_loss1] = calculateRiskMetrics(Z, T, N_w, beta, lambda_loss1_t, P_loss1, rho);



%% 风险值计算函数
function [VaR1, CVaR1, F_loss_total] = calculateRiskMetrics(Z, T, N_w, eta, lambda_loss1_t, P_loss1, rho)
    % 计算每个场景下每个用户的损失费用
    ...
    for i = 1:Z
        for t = 1:T
            for k = 1:N_w
                % 计算t时刻用户i在场景k的损失费用
                F_i_loss_t(i, t, k) = lambda_loss1_t(t) * P_loss1(i, t, k);
            end
        end
    ...
    for i = 1:Z
        ...
        % 找到满足累积概率 ≥ eta 的最小损失值（VaR1定义）
        idx = find(cum_prob >= eta, 1, 'first');
        
        if isempty(idx)
            VaR1(i) = sorted_losses(end);  % 若未找到则取最大值
        else
            VaR1(i) = sorted_losses(idx);
        end
    end
    
    % 计算每个用户的CVaR1
    CVaR1 = zeros(Z, 1);  % 初始化CVaR1向量
    
    for i = 1:Z
        user_losses = squeeze(F_i_loss_k(i, :));
        
        % 计算超出VaR1的损失部分 [F_i_loss_k - alpha1]+
...
%         weighted_excess = sum(rho .* excess_loss);
        ...
        % 计算CVaR1
        CVaR1(i) = alpha1 + (1/(N_w * (1 - beta))) *0.5654* sum(F_i_loss_k);
    end
end
%% 目标函数
Obj=0.08*sum(P_e_pv)+0.02*sum(P_batc+P_batd)+sum(pri_e.*P_buy)-sum(grid_sw.*P_sell)...
    -0.1*sum(P_e_cut1)-0.2*sum(P_e_12+P_e_13) + 0.1*CVaR1...
    +0.03*sum(abs(P_e_12)+abs(P_e_13))...
    +sum(0.5*p*(norm(P12d-P_e_12)^2))+sum(lambda_e_12.*(P12d-P_e_12))...
    +sum(0.5*p*(norm(P13d-P_e_13)^2))+sum(lambda_e_13.*(P13d-P_e_13));   
% 设置求解器
fprintf('solver')
...
P_e_cut1=double(P_e_cut1);
Obj_MG1=double(Obj);
P_loss1=double(P_loss1);
total_loss1=double(total_loss1);
CVaR1=double(CVaR1);

%disp(strcat('------【SC Calculate Finished!!】----------'));  
end