function [P_e_31,P_e_32,P_e_cut3,Obj_MG3,P_loss3,total_loss3,CVaR3]=Fun_MG3(P31d,P32d,lambda_e_31,lambda_e_32,p)
%办公中心的分布式优化迭代模型
%% 决策变量初始化
L_e=sdpvar(1,24);       %需求响应后实际的电负荷
P_e_cut3=sdpvar(1,24);  %可削减电负荷
...
P_e_31=sdpvar(1,24);    %3向1交互的电功率
P_e_32=sdpvar(1,24);    %3向2交互的电功率

%% 导入电/热负荷和电网购电电价
%办公中心
L_e0 = [288,237,270,263,229,284,262,512,910,1196,1223,1263,...
       1261,1227,1268,1245,802,751,600,566,492,305,249,264]; 

Predict_pv = [350  340  343  367  354  512  670  713  772  807  855  784 ...
              881  952  938  912  895  840  540  321  386  349  338  335];
...
Tset = ([22.5,22.4,22.2,22.4,22.3,22.1,21.5,20.0,20.8,21.2,21.4,21.8,...
        21.5,21.0,21.6,21.4,21.2,20.9,20.5,20.8,21.6,20.8,22.8,23.1]-18)/7*4+18;     %设定温度
% 计算外壁的表面温度
...
%虚拟储能容量
for t=1:24
    Esum(t) = R_in*c_in*(Tmax-Tset(t))*(V1*15);
    Es(t) = R_in*c_in*(Tset(t)-Tmin)*(V1*15);
end
beta=0.1;
%% 
%微网的电/热负荷需求响应部分
for t=1:24
    C=[C,
       L_e(t)==L_e0(t)+P_e_cut3(t),                  %电负荷功率平衡约束
       -(-0.1*beta+1.01)*Es(t)/3600000<=P_e_cut3(t)<=(-0.1*beta+1.01)*Esum(t)/3600000, %可削减电功率上下限约束
      ];
end
...
for t=1:24
    C=[C,
       P_e_pv(t)+P_buy(t)+P_batd(t)==P_batc(t)+L_e(t)+P_e_31(t)+P_e_32(t)+P_sell(t), 
      ];
end
...
alpha1=0.95;
% 固定惩罚电价（每个时段相同）
lambda_loss3_t = 1 * ones(T, 1);  % 惩罚电价向量

% 给定的实际响应功率偏差数据（24个时段）
base_power_deviation1 = (-0.1*beta+1.01)*[211 206 197 206 201 192 164 93 131 150 159 178 ...
    164 140 168 159 150 136 117 131 168 131 225 239];  % 正整数值

% 生成场景数据（在基准值上添加±10%的随机波动）
P_loss3 = zeros(Z, T, N_w);  % 功率偏差矩阵
rng(42);  % 设置随机种子确保结果可重现
for k = 1:N_w
    % 在基准值上添加随机波动（90%~110%）
    fluctuation = 0.9 + 0.2 * rand(1, T);  % 生成[0.9, 1.1]的随机波动
    P_loss3(1, :, k) = base_power_deviation1 .* fluctuation;
end
...
%% 计算风险指标
[VaR3, CVaR3, total_loss3] = calculateRiskMetrics(Z, T, N_w, beta, lambda_loss3_t, P_loss3, rho);



%% 风险值计算函数
function [VaR3, CVaR3, F_loss_total] = calculateRiskMetrics(Z, T, N_w, eta, lambda_loss3_t, P_loss3, rho)
    % 计算每个场景下每个用户的损失费用
    ...
    for i = 1:Z
        for t = 1:T
            for k = 1:N_w
                % 计算t时刻用户i在场景k的损失费用
                F_i_loss_t(i, t, k) = lambda_loss3_t(t) * P_loss3(i, t, k);
            end
        end
    ...
    for i = 1:Z
        ...
        % 找到满足累积概率 ≥ eta 的最小损失值（VaR3定义）
        idx = find(cum_prob >= eta, 1, 'first');
        
        if isempty(idx)
            VaR3(i) = sorted_losses(end);  % 若未找到则取最大值
        else
            VaR3(i) = sorted_losses(idx);
        end
    end
    
    % 计算每个用户的CVaR3
    CVaR3 = zeros(Z, 1);  % 初始化CVaR3向量
    
    for i = 1:Z
        user_losses = squeeze(F_i_loss_k(i, :));
        
        % 计算超出VaR3的损失部分 [F_i_loss_k - alpha1]+
        excess_loss = max(user_losses - alpha1, 0);
        
        % 计算加权超额损失的期望
%         weighted_excess = sum(rho .* excess_loss);
        
        % 计算CVaR3
        CVaR3(i) = alpha1 + (1/(N_w * (1 - beta))) *0.5654* sum(F_i_loss_k);
    end
end
%% 目标函数
Obj=0.08*sum(P_e_pv)+0.02*sum(P_batc+P_batd)+sum(pri_e.*P_buy)-sum(grid_sw.*P_sell)...
    -0.1*sum(P_e_cut3)-0.2*sum(P_e_31+P_e_32)+0.1*CVaR3...
    +0.03*sum(abs(P_e_31)+abs(P_e_32))...
    +sum(0.5*p*(norm(P31d-P_e_31)^2))+sum(lambda_e_31.*(P31d-P_e_31))...
    +sum(0.5*p*(norm(P32d-P_e_32)^2))+sum(lambda_e_32.*(P32d-P_e_32));    
% 设置求解器
fprintf('solver')
...
P_e_cut3=double(P_e_cut3);
Obj_MG3=double(Obj);
P_loss3=double(P_loss3);
total_loss3=double(total_loss3);
CVaR3=double(CVaR3);
%disp(strcat('------【BGZX Calculate Finished!!】----------'));  
end